    ...
    if (response.status === 200) {
      if (data.count === 0) {
        ...
        for (let i = 0; i < data.jobs.length; i++) {
          ...
          let rowHTML = `<td>${data.jobs[i].title}</td><td>${data.jobs[i].status}</td><td>${data.jobs[i].emotion}</td>${editButton}${deleteButton}`;
    ...
    if (e.target === logoff) {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          emotion.value = data.job.emotion;
          showing.style.display = "none";
          showing = editJob;
          ...
          message.textContent = "The posts entry was not found";
          thisEvent = new Event("startDisplay");
          document.dispatchEvent(thisEvent);
        }
      } catch (err) {
